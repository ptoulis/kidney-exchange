...
get.matching.information <- function(rke, matched.cycles) {
  ...
  count.type.cycles <- function(cycle.size, pair.types) {
    ...
    sum(apply(m, 1, function(xchange.pair.ids) { 
      ...
      CHECK_EQ(length(xchange.pairs$pair.type), length(pair.types))
      all(sort(xchange.pairs$pair.type)==sort(pair.types))
    }))
  }
...
gurobi.matched.pairs <- function(gurobi.result, rke, cycles) {
  ...
  if(is.null(gurobi.result) || is.na(gurobi.result) || 
       gurobi.result$status=="TIME_LIMIT" || gurobi.result$status=="INF_OR_UNBD") {
...
max.matching <- function(rke,
  ...
  if(length(promote.pair.ids) > 0) {
    ...
    model.w <- model.w + 2 * filter.cycles.pair.ids(Cycles, pair.ids=promote.pair.ids)
  }
